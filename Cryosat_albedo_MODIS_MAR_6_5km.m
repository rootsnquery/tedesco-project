...
% the MAR output resolution is 6.5km, not 6km
...
% create equilibrium line altitude (ELA) cutoff at 1679m
SMBH_095 = 1679;
SH_ERA(SH_ERA > SMBH_095) = 0; SH_ERA(SH_ERA ~= 0) = 1;

% group islands. 8 for inclusion of diagonal cells
...
% 19 years, from 2000 to 2021. 92 days, from June 1 to August 31
s_ERA_6km = zeros(230, 415, yrs, 92);
al_ERA_6km = zeros(230, 415, yrs, 92);

% create 4D matrix in size of MAR with 6km resolution for each day
...
for t = 1:yrs
    ...
    % % read SHSN2 and RO1 to compute bare ice extent for each year and day
    % SHSN2 = snow pack height above ice. 0 for no snow or only snow. only gives a
    % value if top 20m has snow and ice
    ...
    % apply DEM height filter, i.e. cells above 1679m cannot be bare ice
    ...
    for is = 1:nfiles
        ...
        % bare ice cells for both MODIS and MAR
